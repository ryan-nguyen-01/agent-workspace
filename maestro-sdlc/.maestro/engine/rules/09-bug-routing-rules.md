...
R-009-06: Every bug must include impacted components when known.
...
R-009-10: Every defect must have one canonical bug artifact under `.maestro/work/bugs/blockers/` or `.maestro/work/bugs/non-blockers/`.
R-009-11: `.maestro/work/tasks/<task-id>/bugs.yaml` is only a task-local index and must reference each canonical bug with `bug_id`, `severity`, `status`, `canonical_path`, and `retest_scope`.
...
.maestro/work/tasks/<task-id>/bugs.yaml          (index/link, not canonical detail)
```
