...
- **English for all framework artifacts**: rules (`.agent/rules/**`), `.agent/workflow.md`, agent and
  command definitions (`.claude/agents/**`, `.claude/commands/**`), templates (`.agent/templates/**`),
  `.agent/docs/**`, entry points (`CLAUDE.md`, `AGENTS.md`, adapters), playbooks, and config comments.
...
- Task artifacts under `.runtime/tasks/**` may quote the user's own words verbatim (their language),
