...
**Role:** Reviews architecture risk after Task Analysis and before implementation planning when a task changes cross-service flows, public APIs, events, schemas, security-sensitive surfaces, infrastructure, or rollout strategy.

**Inputs:** Task analysis, project brain, architecture memory, service catalog, agent registry
...
.runtime/bugs/blockers/<bug-id>.yaml (on blocker)
.runtime/bugs/non-blockers/<bug-id>.yaml (on non-blocker)
```
...
.runtime/context/project-brain.yaml (updated)
.runtime/context/services/<service>.yaml (updated)
.runtime/context/agent-registry.yaml (when scopes change)
```
