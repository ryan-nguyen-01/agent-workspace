...
R-017-02: scope-guard must gate only likely application source. Framework files (.agent/**, .claude/**, scripts/**, docs, root *.md) must NOT be gated, so framework-template maintenance is unaffected.
...
R-017-07: Execution profiles are controlled by env, not by editing hook code:
          ...
          - AW_DISABLED_HOOKS = comma-separated hook ids to disable
...
`AW_HOOK_PROFILE` for that session. Destructive commands always require explicit user approval through
