...
R-007-09: In the standard applied-service pipeline, Dev Verification must write dev-verification.yaml.
...
R-007-13: In the standard applied-service pipeline, only Dev Verification or Coordinator may advance a task/global workflow to DEV_DONE. Coder Leader and generated coders may write coder-results.yaml, but must not claim Code Done by updating workflow-state.yaml or task.yaml to DEV_DONE.
...
```text
.runtime/tasks/<task-id>/dev-verification.yaml   (standard pipeline)
```
