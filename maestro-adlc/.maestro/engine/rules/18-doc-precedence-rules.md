...
`.maestro/engine/docs/doc-precedence.md`; this file is its enforceable form. Adapted from a
Spec-Driven Development tier model.
...
Tiers (full map in `.maestro/engine/docs/doc-precedence.md`):

```text
T0  CLAUDE.md, .maestro/engine/workflow.md, .maestro/engine/rules/**, agent-taxonomy.md   (framework contract)
T1  project.yaml, model-routing.yaml, response-ui.yaml, workflow-state.yaml   (project SSOT)
T2  components.yaml, agents.yaml, test-policy.yaml, skills.yaml, component knowledge files
...
R-018-07: Coder scope contracts in agents.yaml (T2) are authoritative for write scope. A T3
