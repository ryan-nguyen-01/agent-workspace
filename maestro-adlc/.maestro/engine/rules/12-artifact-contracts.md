...
Governed implementation tasks use the full folder below. Assisted tasks require only task.yaml,
progress.yaml, verification.yaml, and continuation memory when needed. Direct tasks may omit it.
...
Framework maintenance may use lightweight evidence instead of the full task folder when all of these are true:
...
no governed product-component source is modified
```
...
```text
Model routing/response UI: `.maestro/config/model-routing.yaml` and `.maestro/config/response-ui.yaml`
Local status/activity: `.maestro/runtime/workflow-state.yaml` and `.maestro/runtime/agent-activity.yaml`
...
Before governed product coding: task-analysis.yaml plus context_plan and any configured approval
Before assisted product coding: task.yaml with bounded scope and verification owner
Before direct product coding: execution-mode classification and explicit verification ownership
...
Before assigning coders in product-component fast-track: lightweight service-assignments.yaml with assignment_note, allowed_write_paths, critical_checks, and context_pack
...
Before routing a defect: canonical .maestro/work/bugs/<severity>/<bug-id>.yaml plus .maestro/work/tasks/<task-id>/bugs.yaml index entry
Before DONE: memory-updates.yaml when durable facts changed
Before splitting a long conversation: .maestro/memory/tasks/<task-id>/handoff.md plus a checkpoint artifact
When verification is user-owned or shared: verification.yaml must list pending user checks and inaccessible evidence
```
...
```text
workflow-state current_state or active_task_id drift from recorded task artifacts
...
Task folder: .maestro/work/tasks/<task_id>/
Epic id: EPIC-YYYYMMDD-NNN-slug
Subtask id: ST-NNN-slug
Design id: <PROJECT>-DESIGN-NNN
Decision id: <PROJECT>-ADR-NNN
...
The `NNN` sequence is per day and should use the next unused number under `.maestro/work/tasks/`.
Do not create task artifacts outside the task folder.
Do not mirror QC handoff outside the task folder; the canonical Dev-to-QC artifact is `.maestro/work/tasks/<task_id>/qc-handoff.md`.
Do not store the only copy of a defect inside `.maestro/work/tasks/<task_id>/bugs.yaml`; it is an index that must point to the canonical `.maestro/work/bugs/.../<bug-id>.yaml` artifact.
