...
R-005-03: In the standard product-component pipeline, Coder Leader must create implementation-plan.yaml before assigning coders.
R-005-04: In the standard product-component pipeline, Coder Leader must create service-assignments.yaml before coder work starts.
R-005-04b: In product-component fast-track, Coder Leader may skip implementation-plan.yaml but must still create a lightweight service-assignments.yaml before coder work starts.
R-005-05: Coder Leader must select coders from agents.yaml.
R-005-06: Coder Leader must reject tasks when no active coder exists for an impacted component.
...
R-005-16: Coder Leader must reject planning when context_plan.confidence is low or unresolved_context contains component boundary, contract ownership, or test policy gaps.
R-005-17: Service assignments must include only the source/memory context each coder needs, not the whole project brain or all component knowledge files.
...
```text
.maestro/work/tasks/<task-id>/implementation-plan.yaml   (standard pipeline)
.maestro/work/tasks/<task-id>/service-assignments.yaml   (standard pipeline or lightweight product-component fast-track)
.maestro/work/tasks/<task-id>/coder-results.yaml         (standard pipeline or product-component fast-track)
.maestro/work/tasks/<task-id>/architecture-review.yaml   (when task-analysis.yaml requires it)
```
