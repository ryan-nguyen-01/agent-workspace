...
discovery, so instead of moving folders this script taxonomises them:
  ...
  4. Writes machine index   -> .maestro/registry/skill-taxonomy.yaml
  5. Writes human catalog    -> .maestro/engine/docs/skill-catalog.md  (quick-selection tables)
...
REGISTRY_START = "# >>> generated skill catalog >>>"
REGISTRY_END = "# <<< generated skill catalog <<<"
...
def main() -> int:
    ...
    entries = []  # (name, category, description, relative_path)
    injected = 0
    for skill_md in sorted(SKILLS.rglob("SKILL.md")):
        ...
        entries.append((name, category, desc, rel_path))
    ...
    for name, category, desc, _ in entries:
    ...
    registry_lines = [
        REGISTRY_START,
        "# Generated by scripts/build-skill-catalog.py; policy overrides above remain hand-maintained.",
        f"catalog_total: {len(entries)}",
        "catalog:",
    ]
    for name, category, desc, rel_path in sorted(entries):
        registry_lines.extend(
            [
                f"  {name}:",
                f"    path: {json.dumps(rel_path, ensure_ascii=True)}",
                f"    category: {json.dumps(category, ensure_ascii=True)}",
                f"    capability: {json.dumps(desc or name, ensure_ascii=True)}",
            ]
        )
    registry_lines.append(REGISTRY_END)
    registry_block = "\n".join(registry_lines)
    ...
    md = [
        ...
        "`.maestro/registry/skill-taxonomy.yaml`. Skills stay physically flat at "
    ...
    if args.check:
        registry_text = REGISTRY_YAML.read_text(encoding="utf-8") if REGISTRY_YAML.is_file() else ""
        pattern = re.compile(
            rf"(?ms)^{re.escape(REGISTRY_START)}$.*?^{re.escape(REGISTRY_END)}$"
        )
        registry_current = bool(
            pattern.search(registry_text)
            and pattern.sub(lambda _: registry_block, registry_text) == registry_text
        )
        print(f"[check] {len(entries)} skills; registry_current={str(registry_current).lower()}.")
        for cat in CATEGORY_LABELS:
            print(f"  {cat:16s} {len(by_cat.get(cat, []))}")
        return 0 if registry_current else 1
    ...
    if pattern.search(registry_text):
        registry_text = pattern.sub(lambda _: registry_block, registry_text)
    else:
        registry_text = registry_text.rstrip() + "\n\n" + registry_block + "\n"
    REGISTRY_YAML.write_text(registry_text, encoding="utf-8")
    print(
        f"Wrote {TAXONOMY_YAML.relative_to(ROOT)}, {REGISTRY_YAML.relative_to(ROOT)}, "
        f"and {CATALOG_MD.relative_to(ROOT)}"
    )
