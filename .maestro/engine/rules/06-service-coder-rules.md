...
R-006-20: Coders verify required inputs before coding (R-021): a backend coder needs the API contract + data model (LLD); a frontend coder needs an approved UI/UX prototype + API contract; coder-data/coder-database/coder-infra need their listed docs (.maestro/engine/docs/input-prerequisites.md). If a prerequisite is missing or insufficient, return `status: blocked, block_reason: missing_prerequisites, missing: [{doc,why,produce_with}]` to Coder Leader and do NOT invent it. Coder Leader routes the gap to the producing agent/command, then re-assigns.
```
