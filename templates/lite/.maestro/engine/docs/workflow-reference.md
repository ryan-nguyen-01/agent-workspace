...
These are the canonical state names from `.maestro/runtime/workflow-state.yaml`.
...
| ONBOARDED | Onboarding | Project brain and component registry exist |
...
NEED_ONBOARDING ──→ ONBOARDED
  Requires: project.yaml, components.yaml, test-policy.yaml
...
READY_FOR_ANALYSIS ──→ ANALYZED
  ...
  For product-component tasks: task-analysis.yaml.context_plan with medium/high confidence
...
ANALYZED ──→ PLANNED
  Requires: task-analysis.yaml with user approval (R-011-10 unless fast-track applies), context_plan present and actionable for product-component tasks

PLANNED ──→ IN_DEV
  Requires: implementation-plan.yaml, service-assignments.yaml
  product-component fast-track may skip implementation-plan.yaml but still requires lightweight service-assignments.yaml
...
| Command         | Agent triggered | Description                             |
| --------------- | --------------- | --------------------------------------- |
| `/sync-memory`  | Memory Update   | Update project brain and component knowledge files |
...
| Creating tests when component policy says no tests | R-011-06 |
...
| Changing workflow policy, state-machine semantics, or persistent workflow-state fields | R-011-13 |
...
Before coding:
  ...
  .maestro/work/tasks/<task-id>/task-analysis.yaml (with user approval)
  .maestro/work/tasks/<task-id>/task-analysis.yaml.context_plan (all product-component tasks)

Before planning architecture-sensitive tasks:
  .maestro/work/tasks/<task-id>/architecture-review.yaml (when required)
...
Before assigning coders:
  ...
  product-component fast-track may omit implementation-plan.yaml but not service-assignments.yaml.
...
Before QC_DONE:
  .maestro/work/tasks/<task-id>/qc-test-results.yaml (zero open blockers)

Before user delivery:
  .maestro/work/tasks/<task-id>/qc-delivery-report.md

Before DONE:
  .maestro/work/tasks/<task-id>/memory-updates.yaml (when durable facts changed)
```
...
| ----- | ------------------ | ------------------------------------------------------------ |
| R-000 | Core               | Classify mode first; governed coding requires task-analysis  |
...
| R-015 | Model/Observability/UI | Model profiles, activity dashboard, response UI |
