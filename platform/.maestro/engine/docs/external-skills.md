...
## Relationship to skills.yaml

```
external-skills.md (this file)           ← Human docs: what was installed, from where, security notes
.maestro/registry/skills.yaml      ← Machine config: selection policy, risk levels, auto-attach rules
```

- **Canonical source for skill selection at runtime** is `skills.yaml`. Agent Factory and Coder Leader read that file to decide which skills to attach.
...
- When adding or updating a skill: use `/skills` and update `skills.yaml`, `skills-lock.json`, this file, and `CHANGELOG.md` when behavior or risk changes.
...
| `test-driven-development`        | `obra/superpowers`                             | TDD when tests are required                           | Only when component test policy allows/requires test files    |
...
Purpose: these skills are pre-installed so Agent Factory can attach the right stack skills when generating component-specific coder agents. Do not attach the entire pack to every coder. Select only the skills that match the detected component stack in Project Brain and Component Knowledge.
...
Purpose: close missing coverage for Java, Go, Rust, TailwindCSS, and MUI. Agent Factory must treat these as optional stack capabilities and attach them only when Project Brain or Component Knowledge detects the matching component stack.
...
Purpose: add coverage for event streaming, message queues, task queues, Redis/cache, Docker, Kubernetes, Terraform, and reverse-proxy/container infrastructure. Agent Factory must attach these skills only when Project Brain or Component Knowledge detects broker/cache/container evidence.
...
- AWS notification pipeline: attach notification-delivery plus cloud-platform-routing plus eventbridge, sqs, lambda, cloudwatch, and component-specific AWS skills.
