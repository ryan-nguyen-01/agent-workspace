---
description: "maestro /ship — Build to a finished product autonomously. `/ship <idea or what to build>` first proposes a clear **direction** and ge..."
argument-hint: "[request or args]"
---

You are running the maestro `/ship` workflow command inside Codex.
...
(`.maestro/engine/`, `.maestro/registry/`, `.maestro/knowledge/`, `.maestro/work/`, `.maestro/runtime/`, `.claude/commands/ship.md`) are present, defer to them as the
authoritative contract — this prompt is a portable mirror.

User input for this command: $ARGUMENTS

---
## Command contract (mirrored from .claude/commands/ship.md)
