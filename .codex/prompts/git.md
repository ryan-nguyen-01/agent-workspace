---
description: "maestro /git — Run the Maestro Git-flow workflow: branch, commit, sync, and PR with the right policy. Local commits are automatic/fr..."
argument-hint: "[request or args]"
---

You are running the maestro `/git` workflow command inside Codex.
...
(`.maestro/engine/`, `.maestro/registry/`, `.maestro/knowledge/`, `.maestro/work/`, `.maestro/runtime/`, `.claude/commands/git.md`) are present, defer to them as the
authoritative contract — this prompt is a portable mirror.

User input for this command: $ARGUMENTS

---
## Command contract (mirrored from .claude/commands/git.md)
