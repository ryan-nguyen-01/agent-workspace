...
- **ADLC-inspired upgrades (3 patterns)** — Học từ source ADLC: (1) **Document precedence tier model** — `.agent/docs/doc-precedence.md` + `R-018`: T0 (CLAUDE.md/workflow.md/rules) > T1 (project-brain/model-routing) > T2 (service-catalog/agent-registry) > T3 (task artifacts); "một quyết định một nguồn chính thức, conflict → tier cao thắng". (2) **Hash-based conflict detection cho generators** — `scripts/_genlib.py` (SHA256 manifest `.runtime/context/.generated.json`, gitignored): generator **từ chối ghi đè file output đã bị sửa tay** trừ khi `--force`; wired vào build-plugin / build-codex-plugin / build-codex-prompts. (3) **Task 2-trục classification** — `task-analysis.yaml.classification`: `class` (slice/integration → độ sâu test) × `cut` (vertical/horizontal-be/horizontal-fe → hình thái cắt) độc lập, ≤3 target/task. Rules 18 → 19.
- **`/access` tool-permission posture (R-011-14)** — Command thứ 17 + `scripts/access-mode.py` để bật `fullaccess` (chạy terminal + đọc/sửa file không bị hỏi permission từng lần) hoặc `guarded` (mặc định). `fullaccess` dùng **permission allowlist** (`permissions.allow` = Bash + file tools), **KHÔNG** dùng `bypassPermissions`/`--dangerously-skip-permissions` — vì bypass bị Claude Code từ chối khi chạy root/sudo; allowlist chạy được cho mọi user (kể cả root) và hook vẫn active. Chỉ đổi `.claude/settings.json` + `workflow-state.yaml.access_mode` — **KHÔNG** đổi workflow approval gates (R-011) hay scope/secret/destructive hooks: cái gì cần hỏi vẫn hỏi, hook vẫn chặn. `/access` Claude-specific (exclude khỏi Codex prompts như `/aw-init`). Áp dụng cho session mới hoặc qua harness permission UI (Shift+Tab). Script tự dọn legacy `defaultMode=bypassPermissions` nếu gặp.
