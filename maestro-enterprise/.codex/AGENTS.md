...
To get the portable workflow commands as real Codex `/` slash commands, install the generated prompts (Claude-only commands like `/access` are excluded):
...
> For the full flow under Codex, clone this repo and work inside a template folder (`templates/<name>/`), or copy a template's contents into your project (see variants/README.md).
