...
```yaml
triggers:
  - Task yêu cầu tạo module/feature mới thuộc pattern phổ biến
  - Agent-coder nhận subtask match một trong các blueprints có sẵn
  - Agent-analyst detect task type → attach blueprint reference vào subtask
...
Location: .agent/blueprints/ (per project) hoặc .claude/skills/skill-role-blueprints/ (global)
```
