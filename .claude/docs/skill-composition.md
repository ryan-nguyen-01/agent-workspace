...
Contextual skills come from Project Brain and Service Brain.
...
2. Coder Leader checks impacted service brains.
3. Coder Leader selects generated coders.
4. Each coder uses required_skills from its agent contract.
5. Each coder adds contextual_skills from service stack.
...
Testing skills when service policy forbids test creation, unless needed for reading existing tests
...
context/services/<service>.yaml wins on service facts
agent-registry.yaml wins on permissions
...
Use that file to map installed skills to `.claude` agents. Install stack-specific skills only after onboarding identifies the actual project stack.
```
