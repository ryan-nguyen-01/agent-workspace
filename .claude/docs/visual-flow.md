...
Purpose: show how `.claude` is organized into definitions, runtime context, task artifacts, and bug tracking.

```text
Definitions: agents, skills, rules, templates, commands, docs
Runtime context: project-brain, service-catalog, agent-registry, test-policy, services, feedback
Task artifacts: task-input through memory-updates per task
Bug tracking: blockers and non-blockers
Root files: CLAUDE.md, workflow.md, README.md, progress.md, changelog.md
```
