...
Tài liệu này giải thích `.claude` dùng để làm gì, khi nào dùng, và bên trong từng folder/file có vai trò gì.

## 1. `.claude` là gì?

`.claude` là **bộ điều hành workflow cho agentic development** của project.

Nó không phải source code ứng dụng. Nó là nơi lưu:

```text
Quy trình làm việc end-to-end
Luật bắt buộc cho agents
Commands để user gọi workflow
Định nghĩa agents
Skills hướng dẫn agent làm việc
Templates cho artifact
Project Brain để nhớ thông tin dự án
Task artifacts
QC handoff
Bug routing
Visual diagrams
...
## 2. Khi nào dùng `.claude`?

Dùng `.claude` khi bạn muốn làm việc theo quy trình agent end-to-end.
...
## 3. Khi nào không nên dùng `.claude`?

Không cần dùng `.claude` cho việc quá nhỏ hoặc không thuộc workflow.
...
Tuy nhiên, nếu task có thể ảnh hưởng project thật, nhiều service, auth, DB, API contract, QC, hoặc cần resume về sau, nên dùng `.claude`.

## 4. Nguyên tắc vận hành

`.claude` được thiết kế theo mô hình:

```text
Workflow -> Rules -> Commands -> Agents -> Skills -> Templates -> Context -> Artifacts
```

Ý nghĩa từng lớp:

```text
workflow.md   State machine và SOP tổng thể
rules/        Luật bắt buộc, không được bỏ qua
commands/     Entry point user gọi
agents/       Vai trò và trách nhiệm của từng agent
skills/       Hướng dẫn thao tác cụ thể cho agent
templates/    Mẫu file artifact chuẩn
context/      Project Brain và memory dùng lại
.tasks/       Artifact theo từng task
handover/     Tài liệu bàn giao dev -> QC
bugs/         Bug blocker/non-blocker
```

Rule priority:

```text
workflow.md định nghĩa state machine
rules/ định nghĩa policy bắt buộc
commands/ định nghĩa cách user gọi workflow
agents/ định nghĩa role behavior
skills/ định nghĩa cách làm chi tiết
```
...
Khi cần hiểu cách `.claude` điều phối tổng thể
...
Khi cần giới thiệu folder `.claude`
...
Theo dõi trạng thái workflow hiện tại.
...
Ghi lại thay đổi quan trọng của workflow `.claude`.
...
Không lưu secret/token/password/log dài vào `.claude`
```
...
| File                      | Vai trò                                                                                 |
| ------------------------- | --------------------------------------------------------------------------------------- |
| `project-brain.yaml`      | Memory tổng thể của project                                                             |
| `service-catalog.yaml`    | Danh sách service/module được phát hiện                                                 |
| `agent-registry.yaml`     | Danh sách coder agents và scope                                                         |
| `test-policy.yaml`        | Quy định unit/manual test theo service                                                  |
| `workflow-state.yaml`     | State hiện tại của workflow                                                             |
| `skill-registry.yaml`     | Registry máy đọc được cho skill selection, risk gate, approval, installed/failed skills |
| `services/<service>.yaml` | Memory chi tiết từng service                                                            |
| `common/generics.md`      | Pattern/util dùng chung để tránh viết lại                                               |

### Trạng thái ban đầu

Sau khi setup, context đang ở trạng thái:
...
Folder lưu bản bàn giao dev -> QC.
...
Dùng sau khi task đạt `DEV_DONE`.
...
-> DEV_DONE
-> /handoff-qc
-> .claude/handover/TASK-123-handoff.md
```
...
QC không được bắt đầu nếu chưa có handoff.
...
```text
Tạo bugs/non-blockers/<bug-id>.yaml
...
-> blocking bug found
-> /bug
-> create bugs/blockers/BUG-xxx.yaml
-> stop QC
...
-> /bug
-> create bugs/non-blockers/BUG-xxx.yaml
-> QC continues
...
```text
.claude/README.md
.claude/docs/folder-guide.md
.claude/docs/visual-flow.md
...
.claude/rules/01-project-brain-rules.md
.claude/rules/11-approval-gates.md
.claude/rules/12-artifact-contracts.md
...
.claude/rules/02-onboarding-rules.md
...
.claude/rules/03-agent-factory-rules.md
...
```text
.claude/rules/06-service-coder-rules.md
...
.claude/rules/08-qc-rules.md
.claude/rules/09-bug-routing-rules.md
...
```text
Thêm file hoặc section trong rules/
...
```text
Thêm template trong templates/
Update rules/12-artifact-contracts.md
Update workflow.md nếu artifact ảnh hưởng state machine
Update visual-flow.md nếu thay đổi luồng
```

Khi thay đổi workflow:

```text
Update workflow.md trước
...
- context/project-brain.yaml deep_project_intelligence
- context/services/<service>.yaml service_deep_intelligence
