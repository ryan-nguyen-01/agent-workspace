...
target_state:
  version: <target version>         # nếu upgrade
  technology: <new tech>            # nếu swap
  pattern: <new pattern>            # nếu refactor
...
3. Phân loại theo risk:
   - HIGH: thay đổi public API, database schema, auth flow
   - MEDIUM: thay đổi internal modules, config
   - LOW: rename, formatting, deprecated warning fixes
...
migration_plan:
  phases:
    ...
    - phase: 2
      name: "Core Migration"
      steps:
        - <từng thay đổi cụ thể, theo thứ tự an toàn>
        - Ưu tiên: config → types/interfaces → implementations → tests
...
tests_after_migration:
  ...
  passed: <n>
  failed: <n>
  new_failures: [<test names nếu có>]

rollback_command: <lệnh rollback nếu cần — e.g. "git revert HEAD~3">
```
