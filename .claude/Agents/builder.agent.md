...
```
Backend language: TypeScript→skill-lang-typescript, Python→skill-lang-python, Go→skill-lang-go
Backend framework: NestJS→skill-framework-nestjs, FastAPI→skill-framework-fastapi, ...
Frontend: React→skill-framework-react, Next.js→skill-framework-nextjs, Vue→skill-framework-vuejs
Database: PostgreSQL→skill-database-postgresql, MongoDB→skill-database-mongodb
ORM: Prisma→skill-database-prisma, TypeORM→skill-database-typeorm
Auth: JWT→skill-auth-jwt, OAuth2→skill-auth-oauth2
Queue: BullMQ→skill-queue-bullmq, Kafka→skill-queue-kafka
DevOps: Docker→skill-devops-docker, GitHub Actions→skill-devops-github-actions
